...
IMPORTANT: For skill checks, use requestSkillCheck tool. Do NOT execute it yourself - wait for the player to roll the dice.`;
    ...
    // Deduplicate tool calls/results globally across all messages to prevent OpenAI duplicate ID errors.
    // Keep HITL state/output parts intact for UI rendering while stripping provider identifiers.
    ...
    const stripIdsFromAnyPart = (part: AnyUIPart): AnyUIPart => {
      if (
        typeof part !== "object" ||
        part === null ||
        (!("id" in part) && !("providerId" in part))
      ) {
        return part;
      }
    ...
    const getPartScore = (part: ToolUIPart): number => {
      const hasOutput =
        "output" in part && (part as { output?: unknown }).output !== undefined;
      const hasResult =
        "result" in part && (part as { result?: unknown }).result !== undefined;
      return (hasOutput ? 2 : 0) + (hasResult ? 1 : 0);
    };

    const hasMeaningfulContent = (msg: UIMessage): boolean => {
      if (!msg || typeof msg !== "object") {
        return false;
      }
      const rawContent =
    ...
    const stripIdsFromMessage = (msg: UIMessage): UIMessage => {
      const sanitizedMsg = { ...msg };
      delete (sanitizedMsg as Record<string, unknown>).id;
      delete (sanitizedMsg as Record<string, unknown>).providerId;
      return sanitizedMsg;
    };

    const stripIdsFromPart = (part: ToolUIPart): ToolUIPart => {
      const hasId =
        (part as { id?: unknown }).id !== undefined ||
        (part as { providerId?: unknown }).providerId !== undefined;
      if (!hasId) {
        return part;
      }
      const toolPart = {
      ...
      delete (toolPart as Record<string, unknown>).id;
      delete (toolPart as Record<string, unknown>).providerId;
      return toolPart;
    };

    const isToolPart = (part: AnyUIPart): part is ToolUIPart => {
      return (
        isToolUIPart(part) &&
    ...
    const classifyToolPart = (part: ToolUIPart): "call" | "result" | null => {
    ...
    const sanitizeMessagesForModel = (
      ...
      const callWinner = new Map<string, PartRef>();
      const resultWinner = new Map<string, PartRef>();
      const orphanResults = new Set<string>();

      // First pass: pick earliest call, best/latest result
      incoming.forEach((msg, msgIndex) => {
        if (!Array.isArray(msg.parts)) {
          return;
        }
        const parts = msg.parts as AnyUIPart[];
        parts.forEach((part, partIndex) => {
          if (!isToolPart(part)) {
            return;
          }
          const toolPart = part;
          const ref: PartRef = { part: toolPart, msgIndex, partIndex };
          const kind = classifyToolPart(toolPart);
          if (kind === "call") {
            const args = (toolPart as { arguments?: unknown }).arguments;
            const hasArgs =
              typeof args === "string" ? args.trim().length > 0 : !!args;
            const hasInput =
              (toolPart as { input?: unknown }).input !== undefined;
            const isSkillCheckCall =
              (toolPart as { toolName?: string }).toolName ===
              "requestSkillCheck";
            if (!hasArgs && !hasInput && !isSkillCheckCall) {
              return;
            }
            if (!callWinner.has(toolPart.toolCallId)) {
              callWinner.set(toolPart.toolCallId, ref);
            }
            return;
          }
          if (kind === "result") {
            const current = resultWinner.get(toolPart.toolCallId);
            if (!current) {
              resultWinner.set(toolPart.toolCallId, ref);
              return;
            }
            const currentScore = getPartScore(current.part);
            const nextScore = getPartScore(toolPart);
      ...
      const keepKey = new Set<string>();
      // Keep at most one call and one result per toolCallId; drop orphan results
      for (const ref of callWinner.values()) {
        keepKey.add(`${ref.msgIndex}:${ref.partIndex}`);
      }
      for (const [toolCallId, ref] of resultWinner.entries()) {
        if (callWinner.has(toolCallId)) {
          keepKey.add(`${ref.msgIndex}:${ref.partIndex}`);
        } else {
          orphanResults.add(toolCallId);
        }
      }
      ...
      const sanitized = incoming
        .filter((msg): msg is UIMessage => {
          ...
          return hasMeaningfulContent(msg);
        })
        .map((msg, msgIndex) => {
          const baseMsg = stripIdsFromMessage(msg);
          ...
          const filteredParts = msg.parts.flatMap<AnyUIPart>(
            (part, partIndex) => {
              if (isToolPart(part as AnyUIPart)) {
                const partKind = classifyToolPart(part as ToolUIPart);
                const key = `${msgIndex}:${partIndex}`;
                if (!keepKey.has(key)) {
                  return [];
                }
                const toolCallId = (part as { toolCallId: string }).toolCallId;
                const seenKey = `${toolCallId}:${partKind ?? "unknown"}`;
                if (seenToolParts.has(seenKey)) {
                  return [];
                }
                seenToolParts.add(seenKey);
                return [stripIdsFromPart(part as ToolUIPart)];
              }
              return [stripIdsFromAnyPart(part as AnyUIPart)];
            }
          );
      ...
      if (orphanResults.size > 0) {
        console.warn(
          "[API] Dropping orphan tool-results with no matching call:",
          Array.from(orphanResults)
        );
      }

      if (sanitized.length === 0) {
        console.warn(
          "[API] Sanitization removed all messages; injecting minimal user poke."
        );
        sanitized.push({
          id: "synthetic-sanitized-message",
          role: "user",
    ...
    const { sanitized: sanitizedMessages } =
      sanitizeMessagesForModel(typedMessages);
    ...
    for (const msg of typedMessages) {
      if (msg.role === "assistant" && msg.parts) {
        ...
        if (toolPartsWithOutput.length > 0) {
          try {
            // Deduplicate tool-result/output parts by toolCallId to avoid repeated saves
            const seenToolCallIds = new Set<string>();
            const dedupedParts =
              Array.isArray(msg.parts) && msg.parts.length > 0
                ? msg.parts.filter((part: unknown) => {
                    if (
                      typeof part === "object" &&
                      part !== null &&
                      "type" in part &&
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer }) => {
        try {
          // Debug: log sanitized tool parts counts before model call
          const debugToolCounts: Record<
            string,
            ...
            if (msg.parts && Array.isArray(msg.parts)) {
              for (const part of msg.parts) {
                if (
                  ...
                  const entry = debugToolCounts[toolCallId] ?? {
                    calls: 0,
                    results: 0,
                  };
                  ...
                  debugToolCounts[toolCallId] = entry;
                }
              }
            }
          }
          ...
          console.log(
            "[API] Sanitized toolCallId counts before model call:",
            debugToolCounts
          );

          // Validate sanitized messages for unique toolCallIds and proper pairing
          const toolCallStats = new Map<
            string,
            { callCount: number; resultCount: number }
          >();
          const callsWithoutResults = new Set<string>();
          const resultsWithoutCalls = new Set<string>();
            ...
            if (msg.parts && Array.isArray(msg.parts)) {
              for (const part of msg.parts) {
                if (
                  ...
                  const stats = toolCallStats.get(toolCallId) ?? {
                    callCount: 0,
                    resultCount: 0,
                  };
                  if (partType === "tool-call") {
                    ...
                    if (stats.callCount > 1) {
                      throw new Error(
                        `Duplicate tool-call for toolCallId: ${toolCallId}`
                      );
                    }
                    ...
                    if (stats.resultCount > 1) {
                      throw new Error(
                        `Duplicate tool-result for toolCallId: ${toolCallId}`
                      );
                    }
                  }
                  toolCallStats.set(toolCallId, stats);
                }
              }
            }
          }

          for (const [toolCallId, stats] of toolCallStats) {
            if (stats.callCount > 0 && stats.resultCount === 0) {
              callsWithoutResults.add(toolCallId);
            } else if (stats.resultCount > 0 && stats.callCount === 0) {
              resultsWithoutCalls.add(toolCallId);
            }
          }

          // Log validation results
          console.log(
            `[API] Message validation: ${toolCallStats.size} unique toolCallIds`
          );
          if (callsWithoutResults.size > 0) {
            console.warn(
              `[API] Calls without results: ${Array.from(
                callsWithoutResults
              ).join(", ")}`
            );
          }
          if (resultsWithoutCalls.size > 0) {
            const missingCalls = Array.from(resultsWithoutCalls).join(", ");
            console.warn(
              `[API] Results without calls (dropping to avoid provider duplicate errors): ${missingCalls}`
            );
            throw new Error(
              `Orphan tool-results detected with no matching tool-call: ${missingCalls}`
            );
          }
