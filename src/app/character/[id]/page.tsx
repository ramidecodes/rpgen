...
import { getCharacterAction, updateCharacterAction, regenerateCharacterPortraitAction } from "@/app/actions/character";
...
import { ArrowLeft, BookOpen, Shield, User, Zap, Save, X, Edit2, RefreshCw, Loader2 } from "lucide-react";
import { useState, useTransition, useEffect } from "react";
import Image from "next/image";
import { useRouter } from "next/navigation";

// This needs to be a client component now because of the editing state
export default function CharacterPageWrapper({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <CharacterDetailView character={data.character} universe={data.universe} />;
}

function CharacterDetailView({ character: initialCharacter, universe }: { character: any; universe: any }) {
  ...
  const [formData, setFormData] = useState({
    ...
    personalityTraits: (initialCharacter.properties?.personalityTraits || []).join(", "),
  });
  ...
  const handleSave = () => {
    startTransition(async () => {
      const traitsArray = formData.personalityTraits.split(",").map((t: string) => t.trim()).filter(Boolean);
  ...
  const handleRegeneratePortrait = () => {
    if (!confirm("This will replace the current portrait. Are you sure?")) return;
            ...
            <div className="md:col-span-4 space-y-6">
                ...
                <CardHeader className="pb-3">
                   {isEditing ? (
                       ...
                       <>
                        ...
                        <CardDescription>{character.properties?.profession}</CardDescription>
                       </>
                   )}
                </CardHeader>
                <CardContent>
                     ...
                     {isEditing ? (
                         ...
                         character.properties?.factionName && (
                           <div className="flex items-center justify-between text-sm">
                              ...
                              <Badge variant="outline">{character.properties.factionName}</Badge>
                           </div>
                         )
                     )}
                   </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                   ...
                   <StatRow label="Scholarship" value={character.stats.scholarship} />
                   <StatRow label="Intuition" value={character.stats.intuition} />
                </CardContent>
              </Card>
            </div>
                    ...
                    <div className="space-y-2">
                        ...
                        {isEditing ? (
                            ...
                            <div className="prose dark:prose-invert text-sm leading-relaxed whitespace-pre-wrap text-muted-foreground">
                                {character.properties?.backstory || "No backstory recorded."}
                            </div>
                        )}
                    </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {isEditing ? (
                                ...
                                <p className="text-sm text-muted-foreground">
                                    {character.properties?.appearance || "No description available."}
                                </p>
                            )}
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {isEditing ? (
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {character.properties?.personalityTraits?.map((trait: string, i: number) => (
